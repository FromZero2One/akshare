...
# https://www.joinquant.com/data/dict/alpha101  函数解释
...
class Alphas(object):
    def __init__(self, df_data):
        ...
        self.volume = df_data['S_DQ_VOLUME'] * 100  # 成交量 可能是因为原始数据的单位是手（1手=100股）
        self.returns = df_data['S_DQ_PCTCHANGE']  # 收益率。通常是收盘价相对于前一个交易日的百分比变化，即涨跌幅。
        # S_DQ_AMOUNT 成交额 单位可能是千元，所以乘以1000转换为元 除以成交量（股数）。注意，分母加了1是为了避免除零。
