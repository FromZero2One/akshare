...
def _trade_summary_line(metrics: dict) -> str:
    ...
    if won or lost:
        return f'交易次数(已闭合): {actual_closed}  胜: {won}  负: {lost}'
...
def strategy_back_trader(symbol: str = "601398", stock_name: str = "", adjust: str = "qfq", tb_df: pd.DataFrame | None = None,
                         ...
                         strategy=BollCross, printlog: bool = False,
    ...
    **strategy_params: 透传给策略的 params（如 period=20, devfactor=2.0）；同时写入 params_json 落库
    """
    if not isinstance(strategy, type):
        raise TypeError("strategy 必须传类引用（如 BollCross），参数用 **strategy_params 传")
...
if __name__ == '__main__':
    """
    测试布林线策略（默认 BollCross + 601398 工行）
    """
    strategy_back_trader(strategy=BollCross)
