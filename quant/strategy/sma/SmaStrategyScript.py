...
def _build_cerebro(
    ...
    # strategy_params 由调用方传入，会作为 cerebro 内部实例化策略时的关键字参数
    ...
    # TODO: A 股印花税（仅卖出时单向收取）—— backtrader 的 BrokerBase.setcommission
    # 不支持 stamp_duty 关键字，需通过自定义 CommInfo 子类实现。
    # 当前：enable_stamp_duty 形参保留但未生效，避免 broker 层误算。
...
def _extract_metrics(strat, startcash: float, fromdate: datetime, todate: datetime) -> dict:
    """
    从 strategy 实例和 analyzer 输出中提取关键指标。
    """
...
def strategy_back_trader(symbol: str = "601398", stock_name: str = "", adjust: str = "qfq", tb_df: pd.DataFrame | None = None,
                         ...
                         strategy=SmaCross, printlog: bool = False,
    ...
    **strategy_params: 透传给策略的 params（如 pfast=5, pslow=20）；同时写入 params_json 落库
    """
    if not isinstance(strategy, type):
        raise TypeError("strategy 必须传类引用（如 SmaCross），参数用 **strategy_params 传")
